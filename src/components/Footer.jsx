        ...
        >
          {CONTACT_ITEMS.map((item) => (
            <a
              key={item.id}
              href={item.href}
              className="flex items-center gap-4 md:gap-6 group p-4 md:p-5 lg:p-6 rounded-3xl bg-gradient-to-r from-green-50 to-blue-50 transition-all duration-500 hover:shadow-lg hover:shadow-[#02C5DF]/5 hover:-translate-y-1"
              target={item.target}
              rel={item.target === "_blank" ? "noopener noreferrer" : undefined}
              aria-label={item.ariaLabel}
            >
              <div className="bg-gradient-to-r from-green-100 to-blue-50 p-2 md:p-3 lg:p-4 rounded-2xl text-black text-lg md:text-xl group-hover:scale-110 transition-transform duration-300 shadow-md shadow-[#02C5DF]/20">
                <item.icon />
              </div>
